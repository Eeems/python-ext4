...
class Inode(Ext4Struct):
    ...
    @property
    def i_size(self) -> int:
        i_size_lo: int = assert_cast(self.i_size_lo, int)  # pyright: ignore[reportAny]
        i_size_high: int = assert_cast(self.i_size_high, int)  # pyright: ignore[reportAny]
        return i_size_high << 32 | i_size_lo

    @property
    def i_file_acl(self) -> int:
        i_file_acl_lo: int = assert_cast(self.i_file_acl_lo, int)  # pyright: ignore[reportAny]
        l_i_file_acl_high: int = assert_cast(self.osd2.linux2.l_i_file_acl_high, int)  # pyright: ignore[reportAny]
    ...
    def fits_in_hi(self) -> bool:
        i_extra_isize: int = assert_cast(self.i_extra_isize, int)  # pyright: ignore[reportAny]
        return (
            self.has_hi
    ...
    def expected_checksum(self) -> int | None:
        ...
        provided_csum: int = 0
        l_i_checksum_lo: int = assert_cast(self.osd2.linux2.l_i_checksum_lo, int)  # pyright: ignore[reportAny]
        provided_csum |= l_i_checksum_lo
        if self.has_hi:
            i_checksum_hi: int = assert_cast(self.i_checksum_hi, int)  # pyright: ignore[reportAny]
...
class Directory(Inode):
    ...
    def _opendir(self):
        ...
        while offset < len(data):
            ...
            rec_len: int = assert_cast(dirent.rec_len, int)  # pyright: ignore[reportAny]
            if not rec_len:
                # How did this happen?
                offset += _type.name.offset  # + EXT4_DIR_ROUND
                continue

            name_len: int = assert_cast(dirent.name_len, int)  # pyright: ignore[reportAny]
            ...
            offset += rec_len
            file_type: EXT4_FT = assert_cast(dirent.file_type, EXT4_FT)  # pyright: ignore[reportAny]
